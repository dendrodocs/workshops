...
using DendroDocs.Uml.Fragments.Extensions;
using PlantUml.Builder.SequenceDiagrams;
using Arrow = DendroDocs.Uml.Fragments.Arrow;

internal class UmlFragmentRenderer
{
    private static readonly string[] ExternalTargets = ["[", "]"];
    ...
    private static void RenderTree(StringBuilder stringBuilder, IEnumerable<InteractionFragment> branch, Interactions tree, List<string?>? activations)
    {
        ...
        foreach (var leaf in branch)
        {
            ...
            switch (leaf)
            {
                ...
                case Arrow arrow:
                    RenderArrow(stringBuilder, arrow, [.. branch], leafActivations);
                    break;

                case Alt alt:
                    leafActivations = [.. branchActivations];
    ...
    private static void RenderGroup(StringBuilder stringBuilder, Alt alt, Interactions tree, List<string?> activations)
    {
        ...
        foreach (var section in alt.Sections)
        {
            ...
            RenderTree(sectionBuilder, section.Fragments, tree, [.. activations]);
    ...
    /// </remarks>
    private static void RenderArrow(StringBuilder stringBuilder, Arrow arrow, IReadOnlyList<InteractionFragment> scope, List<string?> activations)
    {
        var target = (arrow.Source != "W" && arrow.Target == "A") ? "Q" : arrow.Target;

        stringBuilder.Arrow(arrow.Source, $"-{arrow.Color}>", target, message: arrow.Name?.ToSentenceCase());
